...
// function toadd an item to all tables in the database.
CoreMusicLibrary.prototype.addLibraryItem = function(sService, sUri, sTitle, sAlbum, arrayArtists, arrayGenres) {
  ...
  var curItemKey = convertStringToHashkey(sService + sUri);
  ...
  tableItems[curItemKey].children = {};
  tableItems[curItemKey].parents = {};

  var curTrackKey = convertStringToHashkey(sAlbum + sTitle);
  ...
  if (!(curTrackKey in tableTracks)) {
    ...
    tableTracks[curTrackKey].children = {};
    tableTracks[curTrackKey].parents = {};
  }
  ...
  if (sAlbum === 'Greatest Hits') {
    ...
    var curAlbumKey = convertStringToHashkey(arrayArtists[0] + sAlbum);
  } else {
  ...
  if (!(curAlbumKey in tableAlbums)) {
    ...
    tableAlbums[curAlbumKey].children = {};
    tableAlbums[curAlbumKey].parents = {};
  }
  ...
  for (var iArtist = 0; iArtist < arrayArtists.length; iArtist++) {
    var curArtistKey = convertStringToHashkey(arrayArtists[iArtist]);
    ...
    if (!(curArtistKey in tableArtists)) {
      ...
      tableArtists[curArtistKey].children = {};
      tableArtists[curArtistKey].parents = {};
    }
    ...
    for (var iGenre = 0; iGenre < arrayGenres.length; iGenre++) {
      var curGenreKey = convertStringToHashkey(arrayGenres[iGenre]);
      ...
      if (!(curGenreKey in tableGenres)) {
        ...
        tableGenres[curGenreKey].children = {};
        tableGenres[curGenreKey].parents = {};
      }
...
// TODO calls to this function shouldinstead be replaced by 'throw new Error()', which would be caught by
